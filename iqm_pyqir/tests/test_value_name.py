...
def test_setting_function_name_changes_name() -> None:
    ...
    mod = iqm_pyqir.Module.from_ir(context, ir)
...
def test_setting_constant_name_does_not_do_anything() -> None:
    ...
    const0 = iqm_pyqir.const(iqm_pyqir.IntType(context, 64), 42)
    const0.name = "my_value"
    assert const0.name == ""


def test_setting_block_name_changes_name() -> None:
    mod = iqm_pyqir.SimpleModule("test_type_mismatch", 0, 0)
...
def test_int_variable() -> None:
    mod = iqm_pyqir.SimpleModule("test", 0, 0)
    i64 = iqm_pyqir.IntType(mod.context, 64)

    source = mod.add_external_function("source", iqm_pyqir.FunctionType(i64, []))
    sink = mod.add_external_function("sink", iqm_pyqir.FunctionType(i64, [i64]))
...
def test_function_name_can_contain_spaces_and_chars() -> None:
    simple_mod = iqm_pyqir.SimpleModule("test", 0, 0)
