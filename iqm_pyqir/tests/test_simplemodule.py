...
def test_default_attributes_are_set() -> None:
    simple = iqm_pyqir.SimpleModule("test", 2, 5)
    mod = iqm_pyqir.Module.from_bitcode(iqm_pyqir.Context(), simple.bitcode())
...
def test_entry_point_override_is_applied() -> None:
    simple = iqm_pyqir.SimpleModule("test", 2, 5, entry_point_name="new_entry")
    mod = iqm_pyqir.Module.from_bitcode(iqm_pyqir.Context(), simple.bitcode())
