...
import iqm_pyqir
from iqm_pyqir import Context, FunctionType, IntType, PointerType, Type
...
def test_qubit() -> None:
    qubit = iqm_pyqir.qubit_type(Context())
    assert iqm_pyqir.is_qubit_type(qubit)


def test_result() -> None:
    result = iqm_pyqir.result_type(Context())
    assert iqm_pyqir.is_result_type(result)
