...
import iqm_pyqir
import iqm_pyqir.qis
from iqm_pyqir import BasicQisBuilder, Builder, Context, SimpleModule, Type, Value


@pytest.mark.parametrize(
    "name, get_gate",
    [
        ("barrier", lambda: iqm_pyqir.qis.barrier),
    ],
)
...
@pytest.mark.parametrize(
    "name, get_gate",
    [
        ("swap", lambda: iqm_pyqir.qis.swap),
    ],
)
...
@pytest.mark.parametrize(
    "get_value",
    [
        lambda context: iqm_pyqir.const(Type.double(context), 1.0),
        lambda _: 1.0,
    ],
)
def test_rotated(
