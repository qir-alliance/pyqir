...
def _bell(module: Module) -> Function:
    entry = iqm_pyqir.entry_point(module, "bell", 2, 2)
    context = module.context
    builder = Builder(context)
    builder.insert_at_end(BasicBlock(context, "", entry))
    qis = BasicQisBuilder(builder)
    qis.h(iqm_pyqir.qubit(context, 0))
    qis.cx(iqm_pyqir.qubit(context, 0), iqm_pyqir.qubit(context, 1))
    qis.mz(iqm_pyqir.qubit(context, 0), iqm_pyqir.result(context, 0))
    qis.mz(iqm_pyqir.qubit(context, 1), iqm_pyqir.result(context, 1))
    builder.ret(None)
    return entry
...
def _external(module: Module) -> Function:
    ...
    entry = iqm_pyqir.entry_point(module, "barrier", 0, 0)
    builder = Builder(context)
    builder.insert_at_end(BasicBlock(context, "", entry))
...
def test_invalid_module() -> None:
    context = Context()
    module = Module(context, "test")
    main = iqm_pyqir.entry_point(module, "main", 0, 0)
...
def test_prepend_block_with_invalid_parent() -> None:
    context = Context()
    module = Module(context, "test")
    foo = iqm_pyqir.entry_point(module, "foo", 0, 0)
    bar = iqm_pyqir.entry_point(module, "bar", 0, 0)
