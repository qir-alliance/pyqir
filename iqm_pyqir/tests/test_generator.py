...
def test_instruction_lifetime() -> None:
    def make_inst() -> Instruction:
        ...
        v = m.builder.add(x, iqm_pyqir.const(i8, 2))
...
def test_parsed_function_lifetime() -> None:
    def get_entry() -> Function:
        ...
        return next(filter(iqm_pyqir.is_entry_point, m.functions))
...
def test_builder_lifetime() -> None:
    def make_builder() -> Builder:
        ...
        m = Module(c, "test")
        b = Builder(c)
        b.insert_at_end(BasicBlock(c, "", iqm_pyqir.entry_point(m, "main", 0, 0)))
        return b
